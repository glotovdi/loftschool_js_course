...
function deleteTextNodesRecursive(where) {
    deleteTextNodesHelper(where, true);
}
...
function deleteTextNodesHelper(where, isNeedRecursive = false) {
    [...where.childNodes].forEach(node => {
        if (node.nodeType === 3) {
            where.removeChild(node);
        }
        if (isNeedRecursive && node.childNodes.length) {
...
function recursionHelper(fn, node, ...args) {
